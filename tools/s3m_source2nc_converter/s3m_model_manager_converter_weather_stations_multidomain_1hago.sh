...
#-----------------------------------------------------------------------------------------
# Script information
script_name='S3M MODEL - CONVERTER FOR WEATHER STATION DATA - REALTIME MULTIDOMAIN MANAGER'
...
script_folder='/home/fp-s3m/tools/s3m_source2nc_converter/'

# Domain list
domain_name_list=("Valle_Aosta" "Piemonte")

# Most recent input file to look for in order to start
check_file_path_trunk='/home/s3m/national_grid/%Y/%m/%d/'
check_file_name='s3m.italy-grid.rad_rh.%Y%m%d%H%M.nc.gz'
searching_period_hour_obs=2

# Execution example:
# python3 s3m_tool_preprocessing_source2nc_converter.py -settings_file s3m_configuration_preprocessing_sourcenc2nc_converter_weather.json -time "2020-11-02 12:00" -domain "Lombardia"
#-----------------------------------------------------------------------------------------

#-----------------------------------------------------------------------------------------
# Get file information
script_file='/home/fp-s3m/tools/s3m_source2nc_converter/s3m_tool_preprocessing_source2nc_converter.py'
...
if  $file_name_obs_flag ; then
  ...
  for domain_name_name in ${domain_name_list[@]}; do
    ...
    echo " ====> DOMAIN NAME "$domain_name_name" ... "
      ...
      echo " =====> RUN S3m CONVERTER [DOMAIN: $domain_name_name :: TIME: $time_now] ... "
  ...
  echo " =====> MANAGER NOT EXECUTED BECAUSE OF MISSING INPUT FILES!!"
